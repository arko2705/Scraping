...
class Logic:
    ...
    def link_generation(self):##python automatically gives a positional arguement when we call it,so we must write "self"
        ...
        driver=udc.Chrome(version_main=137,use_subprocess=False)##was getting a version mismatch.My chrome
    ...
    def company(self,i,driven):
           ...
           return Company
    def website_address_phonenumber(self,driver):
           ProbableWebSites=[]
           ...
           try:
              ...
              WebSite="Not present on google business profiles"
           try:   
             AFinder=driver.find_element(By.CLASS_NAME,"CsEnBe")
             time.sleep(0.1)
             ...
             if NeedToClean.startswith("Address: "):
              ...
              Address="Not present on google business profiles"
           PFinders=driver.find_elements(By.CSS_SELECTOR,".Io6YTe.fontBodyMedium.kR99db.fdkmkc")
           ...
           for f in PFinders:
             try: 
               ...
               PhoneNumber="Not present on google business profiles"
           return Address,WebSite,PhoneNumber
...
def main():
   ...
   for i in link_list:
      if loop_count<int(loop_number):
         ...
         WAPT=returningThread(target=logic.website_address_phonenumber,args=(GBPdriver,))
         CT.start()
         WAPT.start()
         Company=CT.join()
         Website,Address,PhoneNumber=WAPT.join()
         ET=returningThread(target=logic.email,args=(Company,Website,emaildriver))
         LIT=returningThread(target=logic.linkedin,args=(Website,Company,GBPdriver))
         ET.start()
         LIT.start()
         emaillist=ET.join()
         linkedin=LIT.join()
